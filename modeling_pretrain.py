...
# 'pretrain_videomae_uniformerv2_patch16_224'
...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        if use_learnable_pos_emb:  # 默认为False
            self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim))
        else:
            # sine-cosine positional embeddings 正余弦绝对位置编码
            self.pos_embed = get_sinusoid_encoding_table(num_patches, embed_dim)
        # dpr：0到drop_path_rate的均匀分布
        ...
        if use_learnable_pos_emb:
            trunc_normal_(self.pos_embed, std=.02)  # 初始化为正态分布
    ...
    def forward_features(self, x, mask):
        _, _, T, _, W = x.shape
        x = self.patch_embed(x)  # 4 1568 768 生成长度为1568(8*14*14，表示共有1568个patches)，维度为768(3*16*16)的特征向量。
        x = x + self.pos_embed.type_as(x).to(x.device).clone().detach()

        B, _, C = x.shape

        x_vis = x[~mask].reshape(B, -1, C)  # ~mask means visible 4 160 768

        if self.use_checkpoint:
            for blk in self.blocks:
                x_vis = checkpoint.checkpoint(blk, x_vis)
        else:
            for blk in self.blocks:  # x_vis -> Batch, 49, Channel
                x_vis = blk(x_vis)
