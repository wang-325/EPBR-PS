...
class DropPath(nn.Module):  # 对参数的随机灵活  增加模型的稳健性， 增加模型的泛化能力（解决过拟合问题）
...
class EpbrAttention(nn.Module):
    def __init__(
        ...
        self.softmax_k = nn.Softmax(dim=1)
    ...
    def forward(self, x):
        ...
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1))  # transpose :转置

        attn = attn.softmax(dim=-1)  # softmax是将张量按照某个维度的每个元素缩放到（0,1）区间，且和为1。
        attn = self.attn_drop(attn)  # dropout防止过拟合
        ...
        alph = 0.65
        x = alph * x + (1 - alph) * x_gc
        return x


class EpbrBlock(nn.Module):
    ...
    def forward(self, x):
        if self.gamma_1 is None:
            ...
            # x = torch.nn.functional.adaptive_avg_pool3d(x)
