...
def get_args():
    ...
    parser.add_argument('--data_set', default='Kinetics-400',
                        choices=['NTU', 'ABNTU', 'Kinetics-400', 'SSV2', 'UCF101', 'HMDB51', 'AVA', 'image_folder'],
    ...
    parser.add_argument('--freeze_layers', default=False)
...
def main(args, ds_init):
    ...
    # 相同的随机种子seed将模型在初始化过程中所用到的随机数全部固定下来
    ...
    if args.num_sample > 1:
        collate_func = partial(multiple_samples_collate, fold=False)  # 自定义数据堆叠过程
    else:
        collate_func = None
    ...
    if args.finetune:
        ...
        # interpolate position embedding 位置嵌入，直接载入预训练模型的位置
    ...
    if args.eval:
        ...
        if global_rank == 0:
            ...
            print(
                f"Accuracy of the network on the {len(dataset_test)} test videos: Top-1: {final_top1:.3f}%, Top-5: {final_top5:.3f}%")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            print(f"Accuracy of the network on the {len(dataset_val)} val videos: {test_stats['acc1']:.3f}%")
            ...
            log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                         'epoch': epoch,
                         'n_parameters': n_parameters}
        log1_stats = {'val_Max_accuracy': max_accuracy, 'epoch': epoch}
        if args.output_dir and utils.is_main_process():
            if log_writer is not None:
                log_writer.flush()
            with open(os.path.join(args.output_dir, "log.txt"), mode="a", encoding="utf-8") as f:
