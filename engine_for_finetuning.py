...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if loss_scaler is None:
            ...
            # assert torch.isinf(optimizer).sum() == 0, print(optimizer)
            # print(grad_norm)
        ...
        _, train_predicted = torch.max(probs, dim=1)
        train_acc += (torch.eq(targets, output).sum().item())

        writer.add_scalar('train_loss', loss_value, epoch)
        # writer.add_scalar('train_acc', class_acc, epoch)

        writer.add_scalar('norm', grad_norm, epoch)
        ...
        if log_writer is not None:
            ...
            log_writer.set_step()
    train_accuray = train_acc / train_len
    writer.add_scalar('train_acc', train_accuray, epoch)
